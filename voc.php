...
if ($session != "") {
    ...
    if ($chat_type == "tail") {
        ...
        $shower = "js_writer.php?session=$session";
    }
    ...
    if ($room_id != $room and $user_invisible != 1) {
        ...
        $messages_to_show[] = array(
            ...
            MESG_TOSESSION => "",
            MESG_TOID => "",
            MESG_BODY => "<font color=\"$def_color\">".str_replace(
                    "*",
                    $rooms[$room]["title"],
                    str_replace("~", $user_name, $sw_goes_to_room)
                )."</font>"
        );
        ...
        $messages_to_show[] = array(
            ...
            MESG_BODY => "<font color=\"$def_color\">".str_replace(
                    "*",
                    $rooms[$room_id]["title"],
                    str_replace("~", $user_name, $sw_came_from_room)
                )."</font>"
        );
...
if (check_ban(
    array(
        "un|".to_canon_nick($user_name),
        "ip|".$REMOTE_ADDR,
        "ch|".$c_hash,
        "bh|".$browser_hash,
        "sn|".substr($REMOTE_ADDR, 0, strrpos($REMOTE_ADDR, ".")),
        "sn|".substr($REMOTE_ADDR, 0, strrpos($REMOTE_ADDR, ":"))
    )
)) {
    $error_text = $w_banned;
...
if (!$registered_user) {
    if ($club_mode) {
        header(
            "Location: ".$chat_url."registration_form.php?design=$design&user_name=$user_name&new_user_sex=$new_user_sex&user_color=$user_color&room=$room"
        );
        exit;
    } else {
        ...
        if (!$impro_registration) {
            ...
            if ($reg_word == "true") {
                ...
                header(
                    "Location: ".$chat_url."registration_cell.php?design=$design&user_name=$user_name&new_user_sex=$new_user_sex&user_color=$user_color&room=$room"
                );
                exit;
            }
        }
...
//DD Levandovka fix :-)
if ($current_user->user_class == 0 and $TryToBeInvisible) {
    $TryToBeInvisible = false;
}

if ($current_user->custom_class != 0) {
    $user_custom_class = $current_user->custom_class;
}
...
if (disk_free_space($data_path) < 10 * 1024 * 1024 and is_file($data_path."engine/files/guardian.php") and intval(
        $vocplus_useguardian
    )) {
...
if (isset($_SERVER["HTTP_USER_AGENT"])) {
    $current_user->user_agent = htmlspecialchars($_SERVER["HTTP_USER_AGENT"]);
} else {
    $current_user->user_agent = htmlspecialchars($HTTP_SERVER_VARS['HTTP_USER_AGENT']);
}
...
if (!$fp1) {
    trigger_error(
        "Could not open ".$data_path."similar_nicks.tmp"." for writing. Please, check permissions",
        E_USER_ERROR
    );
}
...
if (!$TryToBeInvisible) {
    ...
    if ($toDay == $birthDay) {
        $messages_to_show[] = array(
            ...
            MESG_TOSESSION => "",
            MESG_TOID => "",
            MESG_BODY => "<font color=\"".$registered_colors[$highlighted_color][1]."\">".str_replace(
                    "~",
                    $user_name,
                    $sw_rob_hb
                )."</font>"
...
{
    global $file_path, $design, $w_title, $user_name, $current_design;

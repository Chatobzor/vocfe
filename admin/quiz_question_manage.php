...
if ($quiz_config['db_server'] && $quiz_config['db_user'] && $quiz_config['db_pass'] && $quiz_config['db_name'] && $quiz_config['db_prefix']) {
	if (mysql_connect($quiz_config['db_server'], $quiz_config['db_user'], $quiz_config['db_pass'])) {
	    ...
	    if (mysql_select_db($quiz_config['db_name'])) {
            ...
            if ($question && $answer) {
            	$sql = 'INSERT INTO '.$quiz_config['db_prefix'].'quiz SET question="'.addslashes($question).'", answer="'.addslashes($answer).'"';
            	mysql_query($sql);
            	header('Location: quiz_question_manage.php?session='.$session.'&page='.$page);
            	exit;
            }
...
if ($error) $error = '<center><b>'.$error.'</b></center>';
...
for ($i = 0; $i < $page_count; $i++) {
	...
	else $pages[] = '<a href="quiz_question_manage.php?session='.$session.'&page='.$i.'">'.($i+1).'</a>';
}
$page_in_row = 11;
$offset = intval($page-floor($page_in_row/2));
if ($offset < 0) $offset = 0;
$length = $page_in_row;
$page_string = '<a href="quiz_question_manage.php?session='.$session.'&page=0">&laquo; первая страница</a> | '.join(' | ', array_slice($pages, $offset, $length)).' | <a href="quiz_question_manage.php?session='.$session.'&page='.($page_count-1).'">последняя страница &raquo;</a>';

?>
<script type="text/javascript">
function about() {
    str = 'Voodoo Chat Quiz engine\n\nVersion: 1.0 final\nAuthor: Mihail Uljanov\nSupport: ICQ 334263747';
    alert(str);
}
</script>
<!--
...
<span style="border-right:1px solid #9d9da1; padding-left:3px; padding-right:2px; cursor:pointer;" onclick="window.location.assign('quiz.php?session=<?=$session;?>')">Настройки </span>
<span style="border-right:1px solid #9d9da1; padding-left:3px; padding-right:2px; cursor:pointer;" onclick="window.location.assign('quiz_question_manage.php?session=<?=$session;?>')">Управление вопросами </span>
<span style="padding-left:3px; padding-right:2px; cursor:pointer;" onclick="about();">О программе </span>
</div>
...
<tr>
  <td>Вопрос:</td>
  <td style="padding-right:30px;"><input type="text" name="question" style="width: 300px;" class="input"></td>
  <td>Ответ:</td>
  <td style="padding-right:30px;"><input type="text" name="answer" style="width: 120px;" class="input"></td>
  <td><input type="submit" value="Добавить" class="button"></td>
</tr>
</table>
</form>
</fieldset>
<div style="text-align:center; padding-top:5px; padding-bottom:5px;"><?php echo $page_string;?></div>
<table width="100%" cellspacing="0" cellpadding="3" style="border-collapse:collapse;" border="1" bordercolor="#728d94">
<tr>

...
function write_to_file($array)
{
    ...
    if (!$fp) {
        trigger_error("Could not open $data_path/items_types.dat for writing. Please, check permissions", E_USER_ERROR);
    }
    if (!flock($fp, LOCK_EX)) {
...
switch ($do) {
    case 'add':
        if (is_numeric($id) && !empty($id) && !empty($title)) {
    ...
    case 'edit':
        ?>
        <div align=center><font size=5 face="Verdana, Tahoma" color="#265D92"
                    align=CENTER><b><?= $adm_shop_manager_cats; ?></b></font></div>
        <table align="center" cellpadding="3">
            <tr>
                <form method="post">
                    ...
                    <td align="center"><input type="Submit" class="button_small" value="<?= $adm_shop['Op_Edit'] ?>">
                    </td>
                </form>
            </tr>
        </table>
...
?>
<div align=center><font size=5 face="Verdana, Tahoma" color="#265D92" align=CENTER><b><?= $adm_shop_manager_cats; ?></b></font>
</div>
<table align="center" cellpadding="3">
    <tr>
    ...
    foreach ($category_list as $id => $title) {
        ...
        <Tr>
            ...
            <td align="center"><strong><a
                            href="?lang=<?= $lang ?>&session=<?= $session ?>&do=edit&id=<?= $id ?>"><?= $adm_shop['Op_Edit'] ?></a>&nbsp;<a
                            href="?lang=<?= $lang ?>&session=<?= $session ?>&do=remove&id=<?= $id ?>"><?= $adm_shop['Op_Del'] ?></a></strong>
            </td>
        </TR>
        <?
    }

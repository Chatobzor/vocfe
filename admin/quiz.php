...
function license_accepted() {
    if(document.getElementById('lic_').checked == false) {
        document.getElementById('lic_text').style.display = 'block';
        return false;
    } else {
        return true;
    }
 ...
 *
 * QUIZ Script for
 * Voodoo Chat & VOC++ Busness Edition
 * Version: 3.0
 * Administration Script
 *
 * @author: ChatMaster <chat@dream.lv>
 * @copyright ChatMaster, 2006-2007
 *
 * Данный скрипт ЗАПРЕЩАЕТСЯ использовать
 * без договорённости с автором
 * в чатах, которые хостятся в Латвии
 * или целевой аудиторией которых является
 * Латвия
 *
 ********************************************
-->
<? if(!$popup): ?>
<!-- About -->
<div class="hidden" id="about_div" style="cursor:default;">
  <div style="background:#eeeeee; padding-bottom:5px; cursor:default;">
    <div style="margin:3px; float:right;"><img src="quiz/images/del.png" width="16" height="16" alt="Close" onclick="about_close();"></div>
    <div style="height:16px; margin:3px; font-size:8px; padding-top:3px; color:gray;">Licenced to: <?=$licenced_to;?></div>
  </div>
  <table align="center" style="margin-top:30px;">
  <tr>
    <td colspan="2" style="text-align:center; font-weight:bold;">VOC++ Quiz Engine</td>
  </tr>
  <tr>
    <td>Version:</td>
    <td>3.0</td>
  ...
  <tr>
    <td>E-Mail:</td>
    <td><a href="mailto:chat@dream.lv" style="color:blue; text-decoration:underline;">chat@dream.lv</a></td>
  </tr>
  </table>
</div>
<!-- End About -->
...
<tr>
  ...
  <td class="head" style="text-align:right;">
   ...
   <tr>
     ...
     <td style="padding-right:3px;"><input type="text" name="kw" value="" id="searchfield" class="input"></td>
...
<tr>
  <td class="tab_bar_bg" colspan="2">
    <table cellspacing="0" cellpadding="0" border="0">
    <tr>
      <td <? if($mod == 'config') {    ?>class="menu_btn_active"<? } else { ?>class="menu_btn" onmouseover="this.className='menu_btn_hl';" onmouseout="this.className='menu_btn';"<? } ?> onclick="window.location.assign('quiz.php?session=<?=$session;?>&m=config');">Настройки</td>
      <td <? if($mod == 'stat') {      ?>class="menu_btn_active"<? } else { ?>class="menu_btn" onmouseover="this.className='menu_btn_hl';" onmouseout="this.className='menu_btn';"<? } ?> onclick="window.location.assign('quiz.php?session=<?=$session;?>&m=stat');">Статистика</td>
      <td <? if($mod == 'questions') { ?>class="menu_btn_active"<? } else { ?>class="menu_btn" onmouseover="this.className='menu_btn_hl';" onmouseout="this.className='menu_btn';"<? } ?> onclick="window.location.assign('quiz.php?session=<?=$session;?>&m=questions');">Вопросы</td>
      <td <? if($mod == 'zerolize') { ?>class="menu_btn_active"<? } else { ?>class="menu_btn" onmouseover="this.className='menu_btn_hl';" onmouseout="this.className='menu_btn';"<? } ?> onclick="window.location.assign('quiz.php?session=<?=$session;?>&m=zerolize');">Обнулить</td>
      <td class="menu_btn" onclick="about();" onmouseover="this.className='menu_btn_hl';" onmouseout="this.className='menu_btn';">О программе</td>
    </tr>
    </table>
  </td>
</tr>
</table>
<div style="background:#edeff8; padding:3px 5px; border-bottom:1px solid #dbdbdb;">&nbsp;<script type="text/javascript" src="<?=QUIZ_SERVER;?>/updates/v3_get_updates.php"></script></div>
<!-- End Menu Tabs -->
<? endif; ?>
...
else echo '<div>Attempt to load unknown module '.$mod_file.'</div>';
?>
<div style="text-align:center; padding-top:10px; padding-bottom:10px;"><small>&copy; ChatMaster, 2006-2008</small></div>

...
if (isset($max_num)) {
    ...
    for ($i = 0; $i <= $max_num; $i++) {
        ...
        $r_id = intval($$param);
        $param = "title_".$i;
        $r_title = prepare_param($param);
        $param = "topic_".$i;
        $r_topic = prepare_param($param);
        $param = "design_".$i;
        $r_design = prepare_param($param);
        $param = "bot_".$i;
        $r_bot = prepare_param($param);
        $param = "pics_".$i;
        $r_pics = (isset($$param)) ? 1 : 0;
        $param = "premoder_".$i;
        $r_premoder = (isset($$param)) ? 1 : 0;
        $param = "club_".$i;
        $r_club = (isset($$param)) ? 1 : 0;
        $param = "pass_".$i;
        $r_pass = prepare_param($param);
        $param = "points_".$i;
        $r_points = (isset($$param)) ? 1 : 0;
...
for ($rg_i = 0; $rg_i < count($room_ids); $rg_i++) {
    ...
    echo "<tr><td><input type=\"text\" class=\"input\" value=\"".str_replace(
            "\"",
            "&quot;",
            $ar_rooms[$room_ids[$rg_i]][ROOM_TITLE]
        )."\" name=\"title_$rg_i\"></td>\n";
    echo "<td><input type=\"text\" class=\"input\" value=\"".str_replace(
            "\"",
            "&quot;",
            $ar_rooms[$room_ids[$rg_i]][ROOM_TOPIC]
        )."\" name=\"topic_$rg_i\"></td>\n";
    ...
    for ($des_i = 0; $des_i < count($all_designes); $des_i++) {
        ...
        if ($all_designes[$des_i] == $ar_rooms[$room_ids[$rg_i]][ROOM_DESIGN]) {
            echo " selected";
        }
    ...
    echo "<td><input type=\"text\" class=\"input\" value=\"".str_replace(
            "\"",
            "&quot;",
            $ar_rooms[$room_ids[$rg_i]][ROOM_BOT]
        )."\" name=\"bot_$rg_i\"></td>";
    echo "<td><input type=\"checkbox\" name=\"premoder_$rg_i\"";
    if ($ar_rooms[$room_ids[$rg_i]][ROOM_PREMODER] == 1) {
        echo " checked";
    }
    echo "></td>\n";

    echo "<td><input type=\"checkbox\" name=\"points_$rg_i\" ";
    if (intval($ar_rooms[$room_ids[$rg_i]][ROOM_POINTS]) == 1) {
        echo "checked";
    }
    echo "></td>";

    echo "<td><input type=\"checkbox\" name=\"club_$rg_i\" ";
    if (intval($ar_rooms[$room_ids[$rg_i]][ROOM_CLUBONLY]) == 1) {
        echo "checked";
    }
    echo "></td>";
    ...
    echo "<td><input type=\"text\" class=\"input\" value=\"".str_replace(
            "\"",
            "&quot;",
            $ar_rooms[$room_ids[$rg_i]][ROOM_PASSWORD]
        )."\" name=\"pass_$rg_i\"></td>";

    echo "<td><input type=\"radio\" name=\"jail\" value=\"".$room_ids[$rg_i]."\" ";
    if (intval($ar_rooms[$room_ids[$rg_i]][ROOM_JAIL]) == 1) {
        echo "checked";
    }
    echo "></td>";
...
<blockquote>
    <span class=txt>*<?php
        echo $adm_predefined_note; ?></span>
    <br><br>
    <span class=txt>**<?php
        echo $adm_premoderated_note; ?></span>
</blockquote>
</body></html>
...
switch ($save_level) {
    case 1:
        if (!isset($conf_file_path)) {
            $conf_file_path = dirname($conf_inc_config)."/";
        }
        ...
        for ($i = 0; $i < count($config_lines); $i++) {
            if (strpos($config_lines[$i], "=")) {
                ...
                if (trim($param) == "\$data_path") {
                    $config_lines[$i] = "\$data_path = \"".$data_path."\";\n";
                }
            }
        }
    ...
    case 3:
        ...
        $chat_types = array();
        for ($j = 0; $j < 4; $j++) {
            if (isset($HTTP_POST_VARS["chat_types"][$j])) {
                $chat_types[$j] = $HTTP_POST_VARS["chat_types"][$j];
            }
        }
    ...
    case 7:
        ...
        $allowed_langs = array();
        for ($j = 0; $j < 50; $j++) {
            if (isset($HTTP_POST_VARS["designes"][$j])) {
                $designes[] = $HTTP_POST_VARS["designes"][$j];
            }
        }
        for ($j = 0; $j < 50; $j++) {
            if (isset($HTTP_POST_VARS["allowed_langs"][$j])) {
                $allowed_langs[] = $HTTP_POST_VARS["allowed_langs"][$j];
            }
        }
...
if ($write_to_conf) {
    ...
    if (!in_array($default_design, $designes)) {
        $default_design = $designes[0];
    }

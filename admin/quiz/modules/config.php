...
if ($ff == "settings") {
    ...
    $configs[] = '$quiz_config[\'short_top_type\'] = "'.trim(
            strip_tags(str_replace("\"", "\\\"", $quiz_short_top_type))
        ).'";';
    $configs[] = '$quiz_config[\'short_top_when\'] = "'.trim(
            strip_tags(str_replace("\"", "\\\"", $quiz_short_top_when))
        ).'";';
...
if (!$error) {
    ...
    if ($do == 'create_tables') {
        ...
        if ($quiz_config['mysql_encoding']) {
            $sql .= ' DEFAULT CHARSET '.$quiz_config['mysql_encoding'];
        }
        mysql_query($sql);
    ...
    /* Check table exists */
    define(
        'QUESTION_TABLE_EXISTS',
        (bool)mysql_query('SELECT 1 FROM '.$quiz_config['db_prefix'].'quiz WHERE 1 LIMIT 1')
    );
    define(
        'TOP_TABLE_EXISTS',
        (bool)mysql_query('SELECT 1 FROM '.$quiz_config['db_prefix'].'quiz_top WHERE 1 LIMIT 1')
    );
    define(
        'STAT_TABLE_EXISTS',
        (bool)mysql_query('SELECT 1 FROM '.$quiz_config['db_prefix'].'quiz_full_stat WHERE 1 LIMIT 1')
    );
} else {
    /* It's fake */
...
switch ($error) {
    ...
    case 3:
        $err = 'Не могу писать в папку '.$data_path.'quiz/';
        break;
    default:
        $err = '';
}
...
foreach ($ar_rooms as $k => $v) {
    ...
    if (in_array(intval($k), $quiz_config['room_ids'])) {
        $rooms_list .= ' selected="selected"';
    }
...
if ($quiz_config['unanswered_type'] != 2) {
    $unanswered_types .= ' selected="selected"';
}
...
<?
if ($err) { ?>
    <div style="margin:10px; padding:15px; text-align:center; color:red; font-weight:bold; border:1px solid red; background:#edeff8;"><?= $err; ?></div><?
} ?>
<?
...
<form action="quiz.php" method="post">
    ...
    <div style="background:#edeff8; border:0px solid #bababa; position:absolute; top:100px; left:30px; width:500px;">
        <div id="lic_text"
                style="color:#000; display:none; background:#ffffff; border:0px solid #bababa; margin:30px; height:340px; padding:10px; overflow:auto;">
    ...
    <fieldset>
        ...
        <table width="100%">
            ...
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                ...
                <td><input type="text" name="tip_timeout" value="<?= intval($quiz_config['tip_timeout']); ?>"
                            style="width: 50;" class="input"> сек. <?= quiz_suggest_config(
                        $quiz_config['tip_timeout'],
                        15
                    ); ?></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left" title="На сколько секунд выходить курить :).">Выходить курить на:</td>
                <td><input type="text" name="smoke_timeout" value="<?= intval($quiz_config['smoke_timeout']); ?>"
                            style="width: 50;" class="input"> сек. <?= quiz_suggest_config(
                        $quiz_config['smoke_timeout'],
                        300
                    ); ?></td>
            </tr>
            ...
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Через сколько секунд после сообщения о неотвеченном задавать следующий вопрос.">Пауза после неотвеченного вопроса:
                </td>
                <td><input type="text" name="unanswered_pause" value="<?= intval($quiz_config['unanswered_pause']); ?>"
                            style="width: 50;" class="input"> сек. <?= quiz_suggest_config(
                        $quiz_config['unanswered_pause'],
                        60
                    ); ?></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Через сколько секунд после сообщения об отвеченном задавать следующий вопрос.">Пауза после отвеченного вопроса:
                </td>
                <td><input type="text" name="answered_pause" value="<?= intval($quiz_config['answered_pause']); ?>"
                            style="width: 50;" class="input"> сек. <?= quiz_suggest_config(
                        $quiz_config['answered_pause'],
                        60
                    ); ?></td>
            </tr>
        </table>
    ...
    <fieldset>
        ...
        <table width="100%">
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Cколько пойнтов начислять пользователю в случае правильного ответа.">Начисляемые пойнты:
                </td>
                <td><input type="text" name="quiz_points" value="<?= $quiz_config['add_points']; ?>" style="width: 50;"
                            class="input"> пойнтов. <?= quiz_suggest_config($quiz_config['add_points'], 300); ?></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Cколько пойнтов отнимать от максимально возможного количества за каждую подсказку.">Цена подсказки:
                </td>
                <td><input type="text" name="tip_price" value="<?= $quiz_config['tip_price']; ?>" style="width: 50;"
                            class="input"> пойнтов. <?= quiz_suggest_config($quiz_config['tip_price'], 100); ?></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Через сколько неотвеченых вопросов бот выходит на перекур.">Выходить на перекур каждые
                </td>
                <td><input type="text" name="max_unanswered" value="<?= intval($quiz_config['max_unanswered']); ?>"
                            style="width: 50;" class="input"> <select name="unanswered_type"
                            class="input"><?= $unanswered_types; ?></select> неотвеченных вопросов
                </td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Автоматически отключаться когда в комнате слишком много людей. 0 - не отключаться">Отключаться когда в комнате больше
                </td>
                <td><input type="text" name="quiz_max_users" value="<?= $quiz_config['max_users']; ?>"
                            style="width: 50;" class="input"> человек
                </td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Показывать правильный ответ когда на вопрос не был дан ответ?">Показывать правильный ответ?
                </td>
                <td><select name="show_correct_answer" class="input"><?= yes_no(
                            $quiz_config['show_correct_answer']
                        ); ?></select></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Комната в которых будут задаваться вопросы и приниматься ответы на вопросы.">Комнаты:<br><small>можно указывать несколько комнат сразу</small>
                </td>
                <td><select name="quiz_room_ids[]" style="width:150;" class="input" multiple="multiple"
                            size="<?= count($ar_rooms); ?>"><?php
                        echo $rooms_list; ?></select></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Ник бота, который задаёт вопросы. Можно указывать ник отличный от ника бота текущей комнаты.">Ник бота:
                </td>
                <td><input type="text" name="quiz_bot_nick" value="<?php
                    echo htmlspecialchars(stripslashes($quiz_config['bot_nick'])); ?>" style="width: 150;"
                            class="input"></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left" title="А чем бот хуже человека? :)">HTML-Ник бота:</td>
                <td>
                    <div id="nick_preview" style="float:right; font-weight:bold; margin-top:3px;"><?php
                        echo $quiz_config['bot_htmlnick']; ?></div>
                    <input id="htmlnick" type="text" name="quiz_bot_htmlnick" value="<?php
                    echo htmlspecialchars($quiz_config['bot_htmlnick']); ?>" style="width: 150;" class="input">
                    <input type="button" class="button" value="Preview" title="Предварительный просмотр HTML-ника бота"
                            style="margin-top:1px;"
                            onclick="document.getElementById('nick_preview').innerHTML = document.getElementById('htmlnick').value;">
                </td>
        </table>
    </fieldset>
    ...
    <fieldset>
        ...
        <table width="100%">
            ...
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                ...
                <td>
                    <table cellspacing="0" cellpadding="0">
                        <tr>
                            <td><input type="text" name="quiz_short_top_cnt" value="<?php
                                echo intval($quiz_config['short_top_cnt']); ?>" style="width:25px;" class="input"></td>
                            <td>
                                <select name="quiz_short_top_type" class="input">
                                    <option value="COUNT">самых умных</option>
                                    <option value="SPEED"<?
                                    if ($quiz_config['short_top_type'] == "SPEED") {
                            ...
                            <td>
                                <select name="quiz_short_top_when" class="input">
                                    <option value="TODAY"<?
                                    if ($quiz_config['short_top_when'] == "TODAY") {
                                        echo ' selected="selected"';
                                    } ?>>сегодня
                                    </option>
                                    <option value="YESTERDAY"<?
                                    if ($quiz_config['short_top_when'] == "YESTERDAY") {
                                        echo ' selected="selected"';
                                    } ?>>вчера
                                    </option>
                                    <option value="WEEK"<?
                                    if ($quiz_config['short_top_when'] == "WEEK") {
                                        echo ' selected="selected"';
                                    } ?>>за неделю
                                    </option>
                                    <option value="MONTH"<?
                                    if ($quiz_config['short_top_when'] == "MONTH") {
                                        echo ' selected="selected"';
                                    } ?>>за месяц
                                    </option>
                                    <option value="ALWAYS"<?
                                    if ($quiz_config['short_top_when'] == "ALWAYS" || !$quiz_config['short_top_when']) {
                                        echo ' selected="selected"';
            ...
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left" title="Выводить !команды в приват вызывающему.">Выводить
                    <i>!команды</i> в приват:
                </td>
                <td><select name="private_output" class="input"><?= yes_no($quiz_config['private_output']); ?></select>
                </td>
            </tr>
        </table>
    </fieldset>
            ...
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                ...
                <td><input type="text" name="db_prefix" value="<?php
                    echo htmlspecialchars(stripslashes($quiz_config['db_prefix'])); ?>" style="width: 150;"
                            class="input"> <?= quiz_suggest_config($quiz_config['db_prefix'], $mysql_table_prefix); ?>
                </td>
            </tr>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left" title="Кодировка MySQL">Кодировка:</td>
                <td><input type="text" name="mysql_encoding" value="<?php
                    echo htmlspecialchars(stripslashes($quiz_config['mysql_encoding'])); ?>" style="width: 150;"
                            class="input"> <?= quiz_suggest_config($quiz_config['mysql_encoding'], 'cp1251'); ?></td>
            </tr>
            <tr class="tr_normal" onmouseover="this.className='tr_hover';" onmouseout="this.className='tr_normal';">
                <td class="settings_left"
                        title="Переподключаться к базе задавая каждый вопрос?">Переподключаться к базе?
                </td>
                <td><select name="need_db_reconnect" class="input"><?= yes_no(
                            $quiz_config['need_db_reconnect']
                        ); ?></select>
                    <small>Ставить "ДА", только если демон викторины периодически падает. Возможно это поможет.</small>
                </td>
            </tr>
        </table>
    </fieldset>

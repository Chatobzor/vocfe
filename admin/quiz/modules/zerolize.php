...
?>
    ...
    <fieldset>
        ...
        <ul>
            ...
            <li>
                <a href="quiz.php?m=zerolize&session=<?= $session; ?>&act=all">Обнулить статические и динамические данные **</a>
            </li>
            <li>
                <a href="quiz.php?m=zerolize&session=<?= $session; ?>&act=user">Обнулить результаты викторины для конкретного пользователя</a>
            </li>
        </ul>
        <div>
            <b>* статические данные</b> - данные за ВСЁ время работы викторины (или с момента последнего удаления статических данных)
        </div>
        <div>
            <b>** динамические данные</b> - данные за последние 30 дней работы викторины. Записи старше 30 дней автоматически удаляются из этого массива данных. По этим данным выстраиваются топы за сегодня/вчера/неделю/месяц.
        </div>
    </fieldset>
        ...
        if (count($users)) : ?>
            ...
            foreach ($users as $k => $v): ?>
                <a href="quiz.php?m=zerolize&session=<?= $session; ?>&act=user&user=<?= $k; ?>" class="user"
                        onclick="if(!confirm('Обнулить все результаты викторины от этого пользователя?')) return false;"><?= $v; ?></a>
            <?
            endforeach; ?>

...
<fieldset>
    ...
    <table>
        <tr>
            ...
            <td><?php
                if (!$is_running) {
                    ...
                    echo 'запущен, PID: '.file_get_contents(
                            $quiz_pid_file
                        ).' <a href="quiz.php?session='.$session.'&m='.$module.'&act=stop" onclick="if (!confirm(\'Эт... а ты хорошо подумал?\')) return false;">[остановить]</a>';
                } ?></td>
        </tr>
        <?
        if ($is_running) { ?>
        ...
        <tr>
            ...
            <td><?php
                echo intval($question_count); ?></td>
        </tr>
        <tr>
            <td width="250" valign="top">Топ пользователей:</td>
            <td>
                <script type="text/javascript" src="<?= $chat_url; ?>quiz_top_export.php"></script>
            </td>
        </tr>
        <tr>
            <td width="250" valign="top">Экспорт топа пользователей:</td>
            <td>
                Для вставки топа пользователей в нужное Вам место используйте указанный ниже код.<br> Настроить внешний вид таблицы Вы можете отредактировав файл
                <b><?= $file_path; ?>quiz_top_export.php</b><br/>
                <textarea
                        style="width:100%; height:40px; color:#5b645b; font-family:Verdana, Tahoma, Arial; font-size:11px;"><?= htmlspecialchars(
                        '<script type="text/javascript" src="'.$chat_url.'quiz_top_export.php"></script>'
                    ); ?></textarea>
            </td>
        </tr>
        <tr>
            ...
            <td>
                Для показа динамических топов пользователям используйте URL: <a href="<?= $chat_url; ?>quiz_tops.php"
                        target="_blank" style="font-weight:bold;"><?= $chat_url; ?>quiz_tops.php</a><br>
                Просто вставьте эту ссылку туда, куда Вам удобно.
            </td>
        </tr>
    </table>
    <input type="button"
            onclick="if (confirm('Это удалит статичный топ пользователей. Хотите продолжить?')) { window.location.assign('quiz.php?session=<?= $session; ?>&m=<?= $module; ?>&del=static'); }"
            value="Обнулить статичные данные" class="zerolize"/><br/>
    <input type="button"
            onclick="if (confirm('Это удалит статичный топ пользователей и все топы за последние 30 дней. Хотите продолжить?')) { window.location.assign('quiz.php?session=<?= $session; ?>&m=<?= $module; ?>&del=full'); }"
            value="Обнулить ВСЕ данные" class="zerolize"/>
</fieldset>
<!-- End Statistics -->
<?php

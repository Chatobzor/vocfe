...
function form_message($last_id, $message, $ignored_users)
{
    ...
    if (intval(trim($cu_array[USER_REDUCETRAFFIC])) > 0) {
        $mesg_array[MESG_BODY] = eregi_replace("<img", "<!-", $mesg_array[MESG_BODY]);
    }

    if ($mesg_array[MESG_ROOM] == $cu_array[USER_ROOM] or $mesg_array[MESG_ROOM] == -1) {
        if ($mesg_array[MESG_ID] > $last_id) {
            if (!isset(
                    $ignored_users[strtolower(
                        $mesg_array[MESG_FROMWOTAGS]
                    )]
                ) || $mesg_array[MESG_FROMWOTAGS] == "") {
                if (strlen($mesg_array[MESG_TO]) == 0) {
                    ...
                    if (!(strpos($mesg_array[MESG_BODY], strtolower($cu_array[USER_NICKNAME])) === false)
                        or ($cu_array[USER_GENDER] == GENDER_GIRL and !(strpos(
                                    $mesg_array[MESG_BODY],
                                    $sw_usr_girls_link
                                ) === false))
                        or (intval($cu_array[USER_CLANID]) == intval($mesg_array[MESG_CLANID]) and intval(
                                $mesg_array[MESG_CLANID]
                            ) > 0 and !(strpos($mesg_array[MESG_BODY], $sw_usr_clan_link."&gt;") === false))
                        or ($cu_array[USER_GENDER] == GENDER_BOY and !(strpos(
                                    $mesg_array[MESG_BODY],
                                    $sw_usr_boys_link
                                ) === false))
                        or ($cu_array[USER_GENDER] == GENDER_THEY and !(strpos(
                                    $mesg_array[MESG_BODY],
                                    $sw_usr_they_link
                                ) === false))
                    ...
                    if (strcmp($mesg_array[MESG_FROMSESSION], $cu_array[USER_SESSION]) == 0 ||
                        (strcmp(
                                $mesg_array[MESG_FROMWOTAGS],
                                $cu_array[USER_NICKNAME]
                            ) == 0 && $mesg_array[MESG_FROMID] > 0)
                    ) {
                        $to_out = str_replace("[HOURS]", date("H", $mesg_array[MESG_TIME]), $private_message_fromme);
                    } else {
                        if (strcmp($mesg_array[MESG_TOSESSION], $cu_array[USER_SESSION]) == 0 ||
                    ...
                    if (strlen($to_out) == 0) {
                        if (($cu_array[USER_GENDER] == GENDER_BOY and $mesg_array[MESG_TO] == $sw_usr_boys_link) or
                            (intval($cu_array[USER_CLANID]) == intval($mesg_array[MESG_CLANID]) and intval(
                                    $mesg_array[MESG_CLANID]
                                ) > 0 and $mesg_array[MESG_TO] == $sw_usr_clan_link) or
                ...
                if (function_exists('preg_replace')) {
                    if (preg_match(
                        "/".$cu_array[USER_NICKNAME]."([".preg_quote("?&:, !")."])/",
                        $mesg_array[MESG_BODY]
                    )) {
                        $nick_in_str = 1;
                    }
                    $mesg_array[MESG_BODY] = preg_replace(
                        "/".$cu_array[USER_NICKNAME]."([".preg_quote("?&:, !")."])/",
                        $nick_highlight_before.$user_name.$nick_highlight_after."\\1",
                        $mesg_array[MESG_BODY]
                    );
                }
                $to_out = str_replace("[MESSAGE]", $mesg_array[MESG_BODY], $to_out);
                if ($nick_in_str) {
                    $to_out = $str_w_n_before.$to_out.$str_w_n_after;
                }
                ...
                if (strlen($to_out) > 0) {
                    ...
                    if ($IsPublic == true) {
                        $to_out = "parent.AddMsgToPublic('".$to_out."', '".addslashes(
                                $mesg_array[MESG_FROMWOTAGS]
                            )."');";
                    } else {
                        if (trim($mesg_array[MESG_FROMWOTAGS]) == "&CMD") {
                            ...
                            $to_out = "parent.AddMsgToPriv('".$to_out."', '".addslashes(
                                    $mesg_array[MESG_FROMWOTAGS]
                                )."', '".addslashes($mesg_array[MESG_TO])."');";
                        }
                    }
                }
            }

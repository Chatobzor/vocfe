...
<?php
if ($is_regist){ ?>
    <a href="board_list.php?session=<?php
    echo $session; ?>"><?php
        echo $w_back_to_userboard; ?></a><br>
    <blockquote>
        <?php
        }
        echo $info_message; ?>
        ...
        <form method="post" action="board_post.php">
            ...
            echo $group; ?>">
            <?php
            echo $w_select_nick; ?>: <select name="send_to_id" class="input">
                <?php
            ...
            <img border="1" src="<?= $current_design ?>profile_captcha.php?<?php
            echo session_name() ?>=<?php
            echo session_id() ?>"></p>
            <?php
            echo $w_subject; ?>: <input type="text" name="subject" size="20" maxlength="50" value="<?php
            echo $tmp_subject; ?>" class="input"><br>
            <?php
            echo $w_message_text; ?>:<br>
    ...
    <form method="post" action="board_send.php">
        ...
        <blockquote>
            ...
            if ($cu_array[USER_CLANID] > 0) {
                echo "<a href=\"board_send.php?session=$session&send_to=6&group=1\">$w_usr_clan_link</a><br>";
            }
            ?>
        </blockquote>
        <?php
        echo $w_not_shure_in_nick."<br>".$w_enter_nick; ?>: <input type="text" name="send_to" maxlength="15"
                class="input">
        <input type="submit" value="<?php
        echo $w_search_button; ?>" class="input">
    </form>
<?php

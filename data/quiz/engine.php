...
while (1) {
    ...
    if (file_exists($last_answ)) {
		...
		if (strlen($data)) {
			...
			if (count($new_top_str) == 4) {
				/* Save current result */
				$sql = 'INSERT INTO '.MYSQL_TABLE_PREFIX.'quiz_top VALUES ('.intval($new_top_str[0]).', "'.mysql_real_escape_string($new_top_str[1]).'", '.intval($new_top_str[2]).', "'.$new_top_str[3].'")';
				mysql_query($sql);
				...
				$sql = 'SELECT * FROM '.MYSQL_TABLE_PREFIX.'quiz_full_stat WHERE user_id='.intval($new_top_str[0]).' LIMIT 1';
				$res = mysql_query($sql);
				$row = mysql_fetch_array($res);
				if (!$row) {
					$sql = 'INSERT INTO '.MYSQL_TABLE_PREFIX.'quiz_full_stat SET user_id='.intval($new_top_str[0]).', user_name="'.mysql_real_escape_string($new_top_str[1]).'", cnt=1, fastest='.intval($new_top_str[2]);
					mysql_query($sql);
				} else {
					$fastest = $row['fastest'];
					...
					$sql = 'UPDATE '.MYSQL_TABLE_PREFIX.'quiz_full_stat SET cnt=cnt+1, fastest='.intval($fastest).' WHERE user_id='.intval($new_top_str[0]);
					mysql_query($sql);
				}
			}
		}
    ...
    foreach ($QUIZ_ROOM_IDS as $room_id) {
	    ...
	    foreach ($users as $user) {
	    	$data = explode("\t", $user);
	    	if ($data[10] == $room_id) $rooms_data[$room_id]++;
		}
	}
	...
	foreach ($rooms_data as $room_id => $cnt) {
		...
		if ($add == true) $quiz_config['room_ids'][] = $room_id;
	}
    ...
    if (file_exists($answer)) {
        ...
        if ($unanswered == $quiz_config['max_unanswered']) {
            ...
            include($engine_path."messages_put.php");
            if (file_exists($bot_online)) unlink($bot_online);
            ...
            $messages_to_show = array(); /* Clear variable */
            foreach ($quiz_config['room_ids'] as $room_id) {
                $messages_to_show[] = quiz_prepare_message(str_replace('<{SEC}>', $quiz_config['unanswered_pause'], $w_quiz_unanswered).' '.$correct_answer);
            }
            include($engine_path."messages_put.php");

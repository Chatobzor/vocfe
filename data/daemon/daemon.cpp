...
if(SUPPORT_MOD_VOC==2) {						
								char  *search_ip, *search_ip_end;
								search_ip = strstr(search_start, "Remote-Addr: ");
								search_ip_end = strstr(search_ip, "\n");
								if (search_ip != NULL && search_ip_end != NULL) {
									size_t ip_to_copy = search_ip_end - search_ip - 1;
									if(ip_to_copy>0) {
										search_ip[ip_to_copy] = '\0'; 
										strcpy(this->remote_ip, search_ip+13);
										snprintf(log_string,sizeof(log_string),"Set Real-IP: %s\n",this->remote_ip);
										my_log(log_string);
									}
								}
}
...
//daemonize closes all out/inc connections :)
        ...
        if (bind(mv_serv, (struct sockaddr *) &mv_uaddr, sizeof (struct sockaddr_un)) == -1) {
...
if(SUPPORT_MOD_VOC==2) {
                                        sin_size = sizeof(struct sockaddr_un);
                                        if ((client_sock = accept(mv_serv, (struct sockaddr *)&mv_client_addr, &sin_size)) == -1) {
                                                snprintf(log_string,sizeof(log_string),"server: cannot accept connection2. %s\n", strerror(errno));
                                                my_log(log_string);
                                                continue;
                                        }
																				...
																				fcntl(client_sock, F_SETFL, O_NONBLOCK);
																				currentC = new Client(client_sock, "socket");
																				if (lastC == NULL) { firstC = currentC; lastC = currentC; currentC->next = NULL; currentC->prev = NULL;}
																				else { lastC->next = currentC; currentC->next = NULL; currentC->prev = lastC; lastC = currentC;}
																				startC = firstC;
																				last_in_list++;
